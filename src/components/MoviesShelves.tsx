...
import { Flame, Eye, Star, Play, Calendar } from 'lucide-react';
...
export const MoviesShelves: React.FC = () => {
  ...
  useEffect(() => {
    ...
    trakt.getMostAnticipatedMovies(10)
      ...
      .catch((err) => {
        console.warn('Error loading anticipated movies:', err);
      })
      .finally(() => {
  ...
  // Load Most Watched Movies (reactive to period change)
  useEffect(() => {
    let isMounted = true;
    setLoadingWatched(true);

    trakt.getMostWatchedMovies(watchedPeriod, 10)
      .then((data) => {
        if (isMounted) setWatched(data);
      })
      .catch((err) => {
        console.warn('Error loading watched movies:', err);
      })
      .finally(() => {
      ...
      <section>
        ...
        {loadingAnticipated ? (
          ...
          <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 touch-pan-x">
            {anticipated.map(renderMovieCard)}
          </div>
        )}
      </section>

      {/* ── 2. Most Watched Movies with Period Selector ── */}
      <section>
